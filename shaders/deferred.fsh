...
vec3 calcSky(in vec2 coord) {
    ...
    vec3 world = normalize(getWorldSpacePositionSky(texcoord.st).xyz);
    ...
    vec3 starColor = vec3(1.0, 1.0, 0.95) * generateStars(world);
    ...
    if (isNight < 0.9) {
        if (timeFactor <= timeFallOffDay) {
            ...
            starColorFinal = starColor * (1.0 - smoothstep(0.0, 0.5, timeFactor / timeFallOffDay));
        } else {
            ...
            starColorFinal = black;
        }
    } else {
        if (timeFactor <= timeFallOffNight) {
            horizonColorBottomFinal = horizonColorBottomMorning * factorMainNight;
            horizonColorTopFinal = horizonColorTopMorning * factorMainNight;
            zenithColorFinal = zenithColorMorning * factorMainNight;

            haloColorDay = haloColorMorning * factorMainNight;
            sunColorFinal = sunColorMorning * factorMainNight;
        } else {
